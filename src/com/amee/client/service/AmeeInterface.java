...
public class AmeeInterface implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5282442695306526974L;
	...
	public boolean signIn() throws AmeeException {
		...
		if (ameeContext.isValid()) {
			final PostMethod post = ameeContext.getPostMethod("/auth/signIn");
			...
			try {
				...
				final Header headers[] = post.getResponseHeaders("authToken");
				if (headers.length > 0) {
					ameeContext.setAuthToken(headers[0].getValue());
					final JSONObject json = new JSONObject(
							post.getResponseBodyAsString());
					final String apiVersion = json.getJSONObject("auth")
							.getString("apiVersion");
					ameeContext.setAPIVersion(apiVersion);
				}

			} catch (final JSONException e) {
				throw new AmeeException("Caught JSONException: "
						+ e.getMessage());
	...
	public String getAmeeResource(final String url) throws AmeeException {
		return getAmeeResource(url, null);
	}

	public String getAmeeResource(final String url,
			final List<Choice> parameters) throws AmeeException {
		GetMethod get = null;
		checkAuthenticated();
		try {
			// prepare method
	...
	public String postAmeeResource(final String url,
			final List<Choice> parameters) throws AmeeException {
		return postOrPutAmeeResource(url, parameters, false);
	}

	public String putAmeeResource(final String url,
			final List<Choice> parameters) throws AmeeException {
		return postOrPutAmeeResource(url, parameters, true);
	}

	public String postOrPutAmeeResource(String url,
			final List<Choice> parameters, final boolean tunnelPut)
		...
		try {
			...
			for (final Choice parameter : parameters) {
			...
			if (location != null) {
				...
				// Add the returnUnit, returnPerUnit parameters to the query
				// string if set in the POST choices.
				String returnUnits = "";
				// TODO - could be more elegant - perhaps pass these into an
	...
	public void deleteAmeeResource(final String url) throws AmeeException {
	...
	private void execute(final HttpMethodBase method) throws IOException,
			AmeeException {
		for (int i = 0; i < ATTEMPTS; i++) {
			...
			case CLIENT_ERROR_BAD_REQUEST:
				// Bad request - check if it's because it's a duplicate and give
				// a more useful error if so.
				// don't retry
				if (method.getResponseBodyAsString().matches(
						".*duplicate resource.*")) {
					throw new AmeeException(
							"The request could not be processed, as it would have created a duplicate resource ("
									+ method.getURI() + ").");
				} else {
					throw new AmeeException(
			...
			case CLIENT_ERROR_NOT_FOUND:
				// don't retry
				throw new AmeeException("The resource could not be found ("
						+ method.getURI() + ").");
			...
			case CLIENT_ERROR_METHOD_NOT_ALLOWED:
				// don't retry
				throw new AmeeException("Method is not allowed ("
						+ method.getURI() + ").");
