...
public class AmeeV2ObjectParser extends AmeeObjectParser {
	...
	@Override
	protected void parseValue(final AmeeValue value, final JSONObject json)
		...
		try {
			...
			if (json.has("profile")) {
				value.setItemRef(new AmeeObjectReference(value.getParentUri(),
	...
	@Override
	protected void parseItem(final AmeeItem item, final JSONObject json)
			throws AmeeException {
		JSONObject itemObj;
		...
		try {
			if (item instanceof AmeeProfileItem) {
				...
				final String dataItemUri = "data/"
						+ UriUtils.getUriExceptFirstTwoParts(profileItem
								.getParentUri()) + "/"
						+ itemObj.getJSONObject("dataItem").getString("uid");
				profileItem.setDataItemRef(new AmeeObjectReference(dataItemUri,
						AmeeObjectType.DATA_ITEM));
			} else {
	...
	@Override
	protected void parseDataCategory(final AmeeDataCategory dataCategory,
			final JSONObject json) throws AmeeException {
		final AmeeV1ObjectParser p = new AmeeV1ObjectParser();
		p.parseDataCategory(dataCategory, json);
	}

	protected void parseReturnValues(final AmeeProfileItem profileItem,
			final JSONObject returnObj) throws JSONException {
		final JSONArray amountsArr = returnObj.getJSONArray("amount");
		for (int i = 0; i < amountsArr.length(); i++) {
			final JSONObject amount = amountsArr.getJSONObject(i);
			profileItem.addReturnValue(new ReturnValue(
					amount.getString("type"), amount.getString("value"), amount
							.getString("unit"), amount.getString("perUnit"),
					amount.getBoolean("default")));
		}
		if (returnObj.has("note")) {
			final JSONArray notesArr = returnObj.getJSONArray("note");
			for (int i = 0; i < notesArr.length(); i++) {
				final JSONObject note = notesArr.getJSONObject(i);
				profileItem.addNote(new ReturnNote(note.getString("type"), note
						.getString("value")));
			}
		}
	}

	protected void parseItemValues(final AmeeItem item,
			final JSONArray valuesArr) throws AmeeException {
		JSONObject valueObj;
		...
		try {
			...
			for (int i = 0; i < valuesArr.length(); i++) {
				...
				ameeObjectReference = new AmeeObjectReference(item.getUri()
						+ "/" + valueObj.getString("path"),
						AmeeObjectType.VALUE);
				item.addValueRef(ameeObjectReference);
				// look for object in cache
				value = (AmeeValue) AmeeObjectFactory.getInstance().getObject(
						ameeObjectReference, false);
				if (value == null) {
	...
	@Override
	protected void parseProfileCategory(
			final AmeeProfileCategory profileCategory, final JSONObject json)
			throws AmeeException {
		...
		try {
			...
			if (json.has("totalAmount")) {
				final Object o = json.get("totalAmount");
				if (o instanceof JSONObject) {
					profileCategory.setAmount(json.getJSONObject("totalAmount")
							.getString("value"));
					profileCategory.setAmountUnit(json.getJSONObject(
							"totalAmount").getString("unit"));
				} else {
	...
	@Override
	protected void parseProfile(final AmeeProfile profile, final JSONObject json)
			throws AmeeException {
	...
	protected void parseCategoryChildren(final AmeeCategory category,
			final JSONObject json) throws AmeeException {
		...
		try {
			if (json.has("profileCategories")) {
				...
				for (int i = 0; i < categoriesArr.length(); i++) {
					...
					if (childJson.has("dataCategory")) {
						...
						ameeObjectReference = new AmeeObjectReference(path
								+ childJson.getJSONObject("dataCategory")
										.getString("path"),
								category.getChildCategoryObjectType());
						category.addCategoryRef(ameeObjectReference);
						final AmeeProfileCategory childProfileCategory = (AmeeProfileCategory) category
								.getNewChildCategory(ameeObjectReference);
						parseProfileCategory(childProfileCategory, childJson);
						AmeeObjectFactory.getInstance().addObjectToCache(
								childProfileCategory);
					} else {
						// create reference and add to parent
						ameeObjectReference = new AmeeObjectReference(category
								.getObjectReference().getPath()
								+ "/"
								+ childJson.getString("path"),
								category.getChildCategoryObjectType());
						category.addCategoryRef(ameeObjectReference);
						// look for object in cache
						childCategory = (AmeeCategory) AmeeObjectFactory
								.getInstance().getObject(ameeObjectReference,
										false);
						if (childCategory == null) {
							...
							if (childCategory instanceof AmeeDataCategory) {
								childCategory
										.setUid(childJson.getString("uid"));
							} else if (childCategory instanceof AmeeDataCategory) {
								childCategory.setUid(""); // no UID for Profile
															// Categories
							}
			...
			final String s = category.getChildItemObjectType().equals(
					AmeeObjectType.DATA_ITEM) ? "dataItems" : "profileItems";
			if (json.has(s) && json.get(s) instanceof JSONArray) {
				...
				for (int i = 0; i < itemsArr.length(); i++) {
					...
					ameeObjectReference = new AmeeObjectReference(category
							.getObjectReference().getPath()
							+ "/"
							+ childJson.getString("uid"),
							category.getChildItemObjectType());
					category.addItemRef(ameeObjectReference);
					// look for object in cache
					childItem = (AmeeItem) AmeeObjectFactory.getInstance()
							.getObject(ameeObjectReference, false);
					if (childItem == null) {
						// create object, add to cache and update
						childItem = category
								.getNewChildItem(ameeObjectReference);
			...
			if (json.has("pager")) {
				...
				if (pagerObj.has("items") && pagerObj.has("itemsPerPage")
						&& pagerObj.has("currentPage")) {
					category.setPage(pagerObj.getInt("currentPage"));
					category.setItemsPager(new Pager(pagerObj.getInt("items"),
							pagerObj.getInt("itemsPerPage"), pagerObj
									.getInt("currentPage")));
				} else {
					category.setPage(1);
			...
			throw new AmeeException("Caught ClassCastException: "
					+ e.getMessage());
		} catch (final JSONException e) {
			throw new AmeeException("Caught JSONException: " + e.getMessage());
		}
	...
	protected void parseCategoryItemProperties(final AmeeItem item,
			final JSONObject json) throws AmeeException {
		AmeeObjectReference ameeObjectReference;
		AmeeValue ameeValue;
		String dataItemUri;
		...
		try {
			...
			if (item instanceof AmeeDataItem) {
				...
				profileItem.setLabel(json.getJSONObject("dataItem").getString(
						"Label"));
				...
				dataItemUri = "data/"
						+ UriUtils.getUriExceptFirstTwoParts(profileItem
								.getParentRef().getPath()) + "/"
						+ json.getJSONObject("dataItem").getString("uid");
				profileItem.setDataItemRef(new AmeeObjectReference(dataItemUri,
						AmeeObjectType.DATA_ITEM));
			}
			// parse values
			// iterate over all keys but only work with keys not in excludes
			// list
			item.clearValueRefs();
			final JSONArray itemValues = json.getJSONArray("itemValues");
			for (int i = 0; i < itemValues.length(); i++) {
				final JSONObject itemValue = itemValues.getJSONObject(i);
				final String path = itemValue.getString("path");
				// create reference and add to parent
				ameeObjectReference = new AmeeObjectReference(item.getUri()
						+ "/" + path, AmeeObjectType.VALUE);
				item.addValueRef(ameeObjectReference);
				// look for object in cache
				ameeValue = (AmeeValue) AmeeObjectFactory.getInstance()
						.getObject(ameeObjectReference, false);
				if (ameeValue == null) {
					// create object and cache
