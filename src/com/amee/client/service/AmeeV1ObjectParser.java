...
public class AmeeV1ObjectParser extends AmeeObjectParser {
	...
	private final static String[] ITEM_EXCLUDES_ARR = { "uid", "path", "label",
			"created", "modified", "dataItemUid", "validFrom", "dataItemLabel",
			"end", "name", "amountPerMonth" };
	private final static List<String> ITEM_EXCLUDES = new ArrayList<String>(
			Arrays.asList(ITEM_EXCLUDES_ARR));
	...
	protected void parseProfileCategory(
		...
		try {
			...
			if (json.has("totalAmountPerMonth")) {
				profileCategory
						.setAmount(json.getString("totalAmountPerMonth"));
			}
			if (json.has("children")) {
				parseCategoryChildren(profileCategory,
						json.getJSONObject("children"));
			}
	...
	protected void parseCategoryItemProperties(final AmeeItem item,
		...
		try {
			...
			if (item instanceof AmeeDataItem) {
				...
				profileItem.addReturnValue(new ReturnValue("amountPerMonth",
						json.getString("amountPerMonth"), "kg", "month", true));
				profileItem.setEnd(json.getString("end"));
				dataItemUri = "data/"
						+ UriUtils.getUriExceptFirstTwoParts(profileItem
								.getParentRef().getPath()) + "/"
						+ json.getString("dataItemUid");
	...
	protected void parseItem(final AmeeItem item, final JSONObject json)
		...
		try {
			if (item instanceof AmeeProfileItem) {
				...
				profileItem.addReturnValue(new ReturnValue("amountPerMonth",
						itemObj.getString("amountPerMonth"), "kg", "month",
						true));
