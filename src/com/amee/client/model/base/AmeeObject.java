...
public abstract class AmeeObject implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1970758397150935804L;
	...
	public AmeeObject(final AmeeObjectReference ref) {
		this();
		setObjectReference(ref);
	}

	public AmeeObject(final String path, final AmeeObjectType objectType) {
		this(new AmeeObjectReference(path, objectType));
	}

	public void populate(final AmeeObject copy) {
		copy.setUid(uid);
		copy.setName(name);
	...
	public void setObjectReference(final AmeeObjectReference ameeObjectReference) {
		if (ameeObjectReference != null) {
			objectReference = ameeObjectReference;
		}
	}
