...
public class CreateProfileItemHistory {
	...
	public static void main(final String[] args) throws AmeeException {
		...
		final String dataItemUID = "66056991EE23"; // The data item UID that you
													...
													// item for.
		final DateTime dt = new DateTime().minusDays(3); // We will start our
															// sequence 3 days
															// ago
		...
		final AmeeProfile profile = AmeeObjectFactory.getInstance()
				.addProfile();
		// Get category
		AmeeProfileCategory cat = AmeeObjectFactory.getInstance()
				.getProfileCategory(profile, profileCategory);

		// We will create three items with different start dates to make a
		// sequence
		// The first two are day long; because they have the same name, they are
		...
		values.add(new Choice("endDate", FMT.print(dt.plusDays(1)))); // Set an
																		// explicit
																		// end
																		// date
		// Create the item
		...
		values.add(new Choice("duration", "P1D")); // Use a duration instead of
													// an end date
		// Create the item
		cat.addProfileItem(dataItemUID, values);
		...
		values.add(new Choice("energyConsumptionUnit", "kWh"));
		values.add(new Choice("energyConsumptionPerUnit", "day"));
		values.add(new Choice("startDate", FMT.print(dt.plusDays(2)))); // Items
																		// without
																		// an
																		// end
																		// date
		...
		cat = AmeeObjectFactory.getInstance().getProfileCategory(profile,
				profileCategory);
